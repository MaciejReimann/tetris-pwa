...
const { concatIfDoesntInclude } = require("./helpers/arrayHelpers");
...
function tetris(prevState, action, callback) {
  ...
  nextScore +=
    (Math.pow(howManyFullRowsBelow(nextSquares, start), 2) * width) / pixel;

  // filer out full rows and drop the rest;
  nextSquares = getSquaresFromNotFullRows(nextSquares).map(square =>
    movePointOnY(square, howManyFullRowsBelow(nextSquares, square) * pixel)
  );
  ...
  function howManyFullRowsBelow(points, point) {
    return yCoordsOfFullRows(points).filter(rowPoint => rowPoint > point.y)
      .length;
  }
  ...
  nextState.squareVertices = [].concat(
    nextState.squares.map(center => getParallelSquareVertices(0, center, pixel))
  );
